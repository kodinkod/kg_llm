...
class GraphRAGChain:
    ...
    def collect_chain(self):
        ...
        self.rag_chain = RunnableParallel(
            {
                "context": self.retriever | get_relevant_docs,
    ...
    def format_chanks(self, docs):
        ...
        for doc in docs:
            ...
            if not (root in exist_root):
                ...
                list_contains = self.graph.query(
                    f"""
                ...
                for list_node in list_contains:
                    ...
                    text = (
                        "информация из списка: Програмный продукт " + l_r + "\n" + l_t
                    )
                    context.append(text)

                # тепрь найдем соседей на 3 близкие чанки к текущему
                neighbord = self.graph.query(
                    f"""
                ...
                for ne in neighbord[:4]:
                    ...
                    if not c_r in exist_root:
                        ...
                        text = (
                            "информация из раздела: Програмный продукт "
                            + c_r
                            + "\n"
                            + c_t
                        )
