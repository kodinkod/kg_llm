...
def get_next_item(num_fmt: str, shift: int):
    """
...
getSuffix = {"nothing": "", "space": " ", "tab": "\t"}


class AbstractNum:
    def __init__(self, tree: BeautifulSoup, styles_extractor: StylesExtractor):
        """
        ...
        try:
            if tree["w15:restartNumberingAfterBreak"]:
                self.properties["restart"] = bool(
                    int(tree["w15:restartNumberingAfterBreak"])
                )
        except KeyError:
    ...
    def parse(self, lvl_list: List[BeautifulSoup]):
        """
        ...
        for lvl in lvl_list:
            ...
            if lvl.lvlText and lvl.lvlText["w:val"]:
                ...
                if hex(ord(lvl.lvlText["w:val"][0])) in mapping:
                    self.levels[ilvl]["lvlText"] = mapping[
                        hex(ord(lvl.lvlText["w:val"][0]))
                    ]
                else:
...
class Num(AbstractNum):
    def __init__(
        ...
        while self.properties["styleLink"]:
            for abstract_num in abstract_num_list.values():
                if abstract_num.find(
                    "w:styleLink", attrs={"w:val": self.properties["styleLink"]}
                ):
    ...
    def get_level_info(self, level_num: str) -> Dict[str, Union[str, bool, int]]:
        """
...
class NumberingExtractor:
    def __init__(self, xml: BeautifulSoup, styles_extractor: StylesExtractor):
        """
        ...
        abstract_num_list = {
            abstract_num["w:abstractNumId"]: abstract_num
        ...
        self.num_list = {
            num_id: Num(num_id, abstract_num_list, num_list, styles_extractor)
            for num_id in num_list
        }

    def _get_list_text(self, ilvl: str, num_id: str) -> str:
        """
        ...
        if (
            self.prev_abstract_num_id
            and self.prev_num_id
            and self.prev_abstract_num_id != abstract_num_id
        ...
        if abstract_num_id in self.prev_ilvl:
            ...
            if (
                prev_ilvl < ilvl
                and lvl_info["lvlRestart"]
                or (abstract_num_id, ilvl) not in self.numerations
            ):
                self.numerations[(abstract_num_id, ilvl)] = lvl_info["start"]
    ...
    def _get_next_number(self, num_id: str, level: str):
        """
    ...
    def parse(
        ...
        if lvl_info["styleId"]:
            self.styles_extractor.parse(
                lvl_info["styleId"], paragraph_properties, "numbering"
            )
        if lvl_info["pPr"]:
