...
from src.docx2graph.from_docx_structure.utils import (
    extract_style, get_GraphDocument_from_triples, get_style_level,
...
@hydra.main(
    version_base=None,
    config_path="../../configs/",
    config_name="docx2graph_advanced_neo4j.yaml",
)
def my_app(cfg: DictConfig) -> None:
    ...
    graph.add_graph_documents([doc], baseEntityLabel=True)
    ...
    for doc_path in tqdm.tqdm(cfg.file_names_for_rag_eval):
        ...
        graph.add_graph_documents(
            [graph_document], baseEntityLabel=True, include_source=True
        )
        ...
        for node in graph_document.nodes:
            if node.type in ["Chunk_node", "Paragraph_node", "List_node"]:
                ...
                graph.query(
                    f"""MATCH (p) WHERE p.id='{node.id}' SET
                            p.roots='{roots}',
                            p.tl_dr='{tl_dr}',
                            p.text='{new_text}'
                            """
